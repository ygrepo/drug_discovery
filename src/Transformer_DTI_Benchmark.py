...
class TransformerModel(nn.Module):
    def __init__(self, drug_input_dim, protein_input_dim, hidden_dim, num_heads, num_layers):
    ...
    def forward(self, drug_features, protein_features):
        drug_embedding = self.drug_encoder(drug_features).unsqueeze(0)  # Add batch dimension
        protein_embedding = self.protein_encoder(protein_features).unsqueeze(0)

        # Transformer expects input as (sequence_length, batch_size, embedding_dim)
        output = self.transformer(drug_embedding, protein_embedding)
        output = output.mean(dim=0)  # Aggregate sequence output
...
for epoch in range(1000):
    ...
    for batch in tqdm(train_loader, desc=f"Training Epoch {epoch + 1}"):
        ...
        optimizer.zero_grad()
        outputs = model(drug_features, protein_features).squeeze()
