...
class EsmLayer(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.add_cross_attention = (
            config.add_cross_attention and (layer_id + 1) % config.adapter_freq == 0
        )
        if self.add_cross_attention:
            ...
            self.LayerNorm_adapter = nn.LayerNorm(
                adapter_config.hidden_size, adapter_config.layer_norm_eps
            )
...
class EsmEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [EsmLayer(config, i) for i in range(config.num_hidden_layers)]
        )
...
class EsmForMutationDesign(EsmPreTrainedModel):
    ...
    def forward(
        ...
        extended_mutation_position = (
            batch_idx * sequence_output.shape[1] + mutation_position
        )
        mutation_aa_logits = self.lm_head(
            sequence_output.view(-1, sequence_output.shape[2])[
                extended_mutation_position
            ]
        )
        ...
        mutation_position_label = torch.zeros(input_ids.shape, dtype=torch.long).to(
            input_ids.device
        )
        mutation_position_label[batch_idx, mutation_position] = 1
        mutation_position_label -= 100 * (1 - attention_mask)

        loss_fct_pos = CrossEntropyLoss(
            weight=torch.tensor([1.0, 50.0]).to(input_ids.device)
        )
        loss_fct_aa = CrossEntropyLoss()
        loss_pos = loss_fct_pos(
            mutation_pos_logits.view(-1, 2), mutation_position_label.view(-1)
        )
        loss_aa = loss_fct_aa(
            mutation_aa_logits.view(-1, self.config.vocab_size), mutation_aa.view(-1)
        )
    ...
    def lm_design(
        ...
        sequence_output = outputs[0]
        mutation_pos_prob = (
            F.softmax(self.mutation_classifier(sequence_output) / 0.5, dim=-1)[:, :, 1]
            * attention_mask
        )
